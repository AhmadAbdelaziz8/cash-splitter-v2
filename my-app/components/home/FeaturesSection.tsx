...
export default function FeaturesSection({ features }: FeaturesSectionProps) {
  return (
    <View className="mt-16">
      ...
      <Text className="text-lg text-slate-500 mb-12 text-center max-w-xl mx-auto px-4">
      ...
      <View className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {features.map((feature, index) => (
          <View
            key={index}
            className="flex-row p-8 bg-white rounded-2xl shadow-lg items-start space-x-6 mx-2 mb-4"
          >
            <View className="flex-shrink-0 w-16 h-16 rounded-full bg-sky-100 items-center justify-center shadow-md gap-2">
              <Ionicons name={feature.icon} size={32} color="#0ea5e9" />
            </View>
            <View className="flex-1">
              <Text className="font-bold text-xl text-slate-700 mb-2">
                {feature.title}
              </Text>
